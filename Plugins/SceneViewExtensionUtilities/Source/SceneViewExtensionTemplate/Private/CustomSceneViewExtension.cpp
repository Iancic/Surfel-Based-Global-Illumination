...
#include "SceneView.h"              // full FSceneView definition
#include "RenderGraphResources.h"   // full FRDGTextureDesc / FRDGTextureRef definitions
#include "RHI.h"                    // full FClearValueBinding definition

// This is the part that connects my MyShader.usf to C++ using the virtual mapping from before
IMPLEMENT_GLOBAL_SHADER(FCustomShader, "/Surfel/MyShader.usf", "MySimpleComputeShader", SF_Compute);
...
void FCustomSceneViewExtension::PrePostProcessPass_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& InView, const FPostProcessingInputs& Inputs)
{
	FRDGTextureDesc outputTexturedesc = FRDGTextureDesc::Create2D(
		InView.UnscaledViewRect.Size(), // width/height in pixels
		PF_FloatRGBA, // pixel format
		FClearValueBinding::Black, // what "cleared" looks like, if you clear it
		TexCreate_UAV | TexCreate_ShaderResource  // usage flags, OR'd together
	);

	// For debug tag (shows in RenderDoc)
	FRDGTextureRef DebugTexture = GraphBuilder.CreateTexture(outputTexturedesc, TEXT("SurfelDebugTexture"));
}
